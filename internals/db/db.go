...
func InitConnectDB() (Conn, error) {
	...
	if err := runMigrations(conn); err != nil {
		log.Fatal("migration unsuccessful%w", err)
	}
...
func (c Conn) SaveProduct(product models.Product) error {
	query := `INSERT INTO Products(name_,price,product_url)
	          VALUES ($1, $2, $3)
...
func (c Conn) PendingProduct() ([]models.Product, error) {
	query := "SELECT id, name, price,product_url FROM Products Where status = 'pending'"
